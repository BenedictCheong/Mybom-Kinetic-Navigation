...
void NaviInterfaceNode::NavStatusCallback(const actionlib_msgs::GoalStatusArray::ConstPtr& status)
{
	...
	if(!status->status_list.empty())
	{
		...
		/*else if (start == 0 && status->status_list[0].status == 3)
		{
			cout << "Finished last movement and wait for a next goal" <<endl;
		}
		else if (start == 0 && status->status_list[0].status>3)
		{
			cout<<"Failed last movement and Not recieved a next goal data yet"<<endl;
		}
		else if (start ==0 && (status->status_list[0].status == 0 || status->status_list[0].status == 2))
		{
			cout<<"not started yet or recieved cancel signal!"<<endl;
			cout<<"Not recieved goal data yet"<<endl;
		}*/

		if ( start == 0 && m_bVelInput == true && status->status_list[0].status != 1)
		{
			start = 1;
			cout <<"already started!! status change missed!!"<<endl;
		}
		
		cout<<"get Msg NavStatus IN!!"<<endl;
		//cout<<status->header.seq<<endl;
		m_status = status->status_list[0];//status->header.seq-3];
		cout<<m_status.status<<endl;
		
		//mPubResult.publish(goalStatus);
