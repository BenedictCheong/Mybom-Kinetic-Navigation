...
bool start_request1 = false;
bool start_request2 = false;
...
void RequestCallback(const ros_robot_controller::msgCommand& msg) {
	start_request1 = msg.command_switch;
	//std::cout << "request!!" << std::endl;
}

void RequestCallback2(const ros_robot_navigation::msgCommand2& msg) {
	start_request2 = msg.command_switch;
	//std::cout << "request!!" << std::endl;
}
...
int main(int argc, char** argv) {
    ...
    ros::Subscriber msg_command_sub = node->subscribe("msgCommand_keyb", 100, &RequestCallback);
    ros::Subscriber msg_command_sub2 = node->subscribe("msgCommand_cmdvel", 100, &RequestCallback2);
    ...
    while(n.ok()) {//n.ok()
        ...
        if(start_request1 || start_request2){state_enable_pub.publish(state_enable_msg);} 
